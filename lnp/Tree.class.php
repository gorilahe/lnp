...
class lnp_Tree {
	...
	public function __construct($data = array(), $id='catid', $parentid='parentid') {
		if($data) {
			$this->makeDataList($data, $id, $parentid);
		}
	}
	...
	public function makeDataList($data = array(), $id='catid', $parentid='parentid'){
		$miniupid = 0;
		foreach($data as $k => $v){
			if($miniupid == 0) $miniupid = $v[$parentid];
	...
	public function getList(&$tree, $root= 0) {
		...
		foreach($this->child[$root] as $key => $id) {
			$tree[] = $id;
			if(isset($this->child[$id]) && $this->child[$id]) $this->getList($tree, $id);
		}
	}
...
$t = new lnp_Tree($data);
var_dump(implode(',',$t->getChildren(6)));